...
if [ -e /home/vagrant/.vbox_version ]; then
...
elif [ -e /home/vagrant/linux.iso ]; then
    apt-get install -y linux-headers-server build-essential
    mount -o loop linux.iso /mnt
    cp /mnt/VM*.tar.gz /tmp
    umount /mnt
    cd /tmp
    tar -zxvf VM*.tar.gz
    ...
    rm -rf /tmp/VM*.tar.gz /tmp/vmware-tools-distrib linux.iso
fi
